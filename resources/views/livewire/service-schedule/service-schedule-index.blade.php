        ...
        <div class="relative overflow-visible bg-white shadow-md  sm:rounded-lg">
            <div class="flex flex-col px-4 py-3 space-y-3 lg:flex-row lg:items-center lg:justify-between lg:space-y-0 lg:space-x-4">
                <div class="flex items-center flex-1 space-x-4">
                    <h5>
                        <span class="text-gray-500">All Service Schedules:</span>
                        <span class="">{{  !empty($service_schedules) ? $service_schedules->total() : 0 }}</span>
                    </h5>
                ...
                <div class="flex flex-col flex-shrink-0 space-y-3 md:flex-row md:items-center lg:justify-end md:space-y-0 md:space-x-3">
                    ...
                    <a href="{{ route('service_schedule.create') }}" 
                        wire:navigate
                ...
                <table class="w-full text-sm text-left text-gray-500 ">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50  ">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3 text-nowrap">Staff</th>  
                            <th scope="col" class="px-4 py-3 text-nowrap">Date (MM DD YYYY)</th>  
                            {{-- <th scope="col" class="px-4 py-3 text-nowrap">Scheduled Days</th>    --}}
                    ...
                    <tbody>
                        @if(!empty($service_schedules) && count($service_schedules) > 0)
                            @foreach ($service_schedules as $service_schedule) 
                                <tr class="border-b  hover:bg-gray-100 ">
                                      <td class="w-4 px-4 py-3">
                                        <div class="flex items-center">
                                            <input id="record-{{ $service_schedule->id }}" 
                                            wire:model="selected_records"
                                            wire:change="updateSelectedCount"
                                            type="checkbox" 
                                            value="{{ $service_schedule->id }}"
                                            class="w-4 h-4 bg-gray-100 border-gray-300 rounded text-primary-600 focus:ring-primary-500 ">
                                            <label for="record-{{ $service_schedule->id }}" class="sr-only">checkbox</label>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap max-w-[10rem] text-wrap overflow-x-auto ">
                                        <div class="flex items-center align-middle  justify-between space-x-2">
                                            ...
                                            <div>
                                                {{ $service_schedule->user->name  ?? null }} 
 
                                                <code class="block">{{ $service_schedule->user->email  ?? null }} </code> 
                                            </div>
                                            ...
                                            <el-dropdown class="inline-block p-0">
                                                ...
                                                <el-menu anchor="bottom end" popover class="m-0 w-56 origin-top-right rounded-md bg-white p-0 shadow-lg outline outline-1 outline-black/5 transition [--anchor-gap:theme(spacing.2)] [transition-behavior:allow-discrete] data-[closed]:scale-95 data-[closed]:transform data-[closed]:opacity-0 data-[enter]:duration-100 data-[leave]:duration-75 data-[enter]:ease-out data-[leave]:ease-in">
                                                    <div class="py-1">
                                                         @if(Auth::user()->hasRole('Global Administrator') || Auth::user()->can('service schedule show') ) 
                                                        <a href="{{ route('service_schedule.show',['service_schedule' => $service_schedule->id]) }}" 
                                                        wire:navigate 
                                                        ...
                                                        @if(Auth::user()->hasRole('Global Administrator') || Auth::user()->can('service schedule edit') )
                                                        <a href="{{ route('service_schedule.edit',['service_schedule' => $service_schedule->id]) }}"  
                                                        wire:navigate 
                                                        class="block px-4 py-2 text-sm text-gray-700 focus:bg-gray-100 focus:text-gray-900 focus:outline-none">
                                                            <div class="flex justify-between items-center">
                                                                <div>
                                                        ...
                                                        <!-- Force Delete-->
                                                        <button
                                                            wire:click="confirmDelete({{ $service_schedule->id }})"
                                                            type="button"
                                    ...
                                    <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap max-w-[20rem] text-wrap overflow-x-auto">
                                        ...
                                        {{-- <div class="flex   gap-2 gap-x-2 ">
                                            <div>
                                                {{ $service_schedule->start_date ? $service_schedule->start_date->format('M d Y') : '' }} 
                                            </div>

                                            <div>
                                                {{ $service_schedule->end_date ? $service_schedule->end_date->format('M d Y') : '' }}
                                            </div>
                                        </div>   --}}
                                        <span class="block my-0">
                                            {{ $service_schedule->schedule_date ? $service_schedule->schedule_date->format('l') : '' }}
                                         
                                        </span>
                                        
                                        <span class="block my-0">
                                            {{ $service_schedule->schedule_date ? $service_schedule->schedule_date->format('M d Y') : '' }}
                                         
                                        </span>

                                        <span class="block my-0">
                                            {{-- {{ $service_schedule->start_time ? $service_schedule->start_time->format('h:i A') : '' }} to {{ $service_schedule->end_time ? $service_schedule->end_time->format('h:i A') : '' }}  --}}
                                         
                                        </span>
                                        
                                    </td>
                                     
                                    {{-- <td class="px-4 py-2 font-medium text-gray-900 whitespace-wrap w-80 ">

                                        @if(!empty($service_schedule->recurring_days))
                                            @foreach ($days = json_decode($service_schedule->recurring_days, true) as $day )
                                                <div class="  uppercase text-sm inline-block bg-sky-700 text-white p-1 m-1 rounded-lg">
                                                    {{ $day }}
                                                </div>
                                    ...
                                    <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap ">
                                        {{  $service_schedule->updated_at->diffForHumans() ?? null }}
                                    </td>
                                    ...
                                    <td class="px-4 py-3 flex items-center justify-end">
                                        ...
                                        <!-- show-service-schedule -->
                                            <a href="{{ route('service_schedule.show',['service_schedule' => $service_schedule->id]) }}" 
                                                data-tooltip-target="tooltip-btn-show-service-schedule-{{ $service_schedule->id }}"
                                            ...
                                            <div id="tooltip-btn-show-service-schedule-{{ $service_schedule->id }}" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-xs opacity-0 tooltip ">
                                                Show Service Schedule
                                        ...
                                        <!-- edit-service-schedule -->
                                            <a href="{{ route('service_schedule.edit',['service_schedule' => $service_schedule->id]) }}" 
                                                data-tooltip-target="tooltip-btn-edit-service-schedule-{{ $service_schedule->id }}"
                                            ...
                                            <div id="tooltip-btn-edit-service-schedule-{{ $service_schedule->id }}" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-xs opacity-0 tooltip ">
                                        ...
                                        <!-- delete-service-schedule -->
                                            <button type="button"   
                                            data-tooltip-target="tooltip-btn-delete-service-schedule-{{ $service_schedule->id }}"
                                            wire:click="delete({{ $service_schedule->id }})" 
                                            wire:confirm.prompt="Are you sure?\n\nType DELETE to confirm|DELETE"
                                            id="record-{{ $service_schedule->id }}-deleteButton"  
                                            ...
                                            <div id="tooltip-btn-delete-service-schedule-{{ $service_schedule->id }}" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-xs opacity-0 tooltip ">
                                                Delete Service Schedule
                                                <div class="tooltip-arrow" data-popper-arrow></div>
                                            </div>
                                        <!-- ./ delete-service-schedule -->
            ...
            @if(!empty($service_schedules))
                <nav class="flex flex-col items-start justify-between p-4 space-y-3 md:flex-row md:items-center md:space-y-0" aria-label="Table navigation">
                    ...
                    <div class="inline-flex items-center gap-x-2">
                        ...
                        <p class="text-sm text-gray-600 ">
                            {{ count($service_schedules) > 0 ? 'of '.$service_schedules->total()  : '' }}
                        </p>
                    </div>
                </nav>
            @endif
