        ...
        <div class="relative overflow-visible bg-white shadow-md  sm:rounded-lg">
            <div class="flex flex-col px-4 py-3 space-y-3 lg:flex-row lg:items-center lg:justify-between lg:space-y-0 lg:space-x-4">
                <div class="flex items-center flex-1 space-x-4">
                    <h5>
                        <span class="text-gray-500">All Permissions:</span>
                        <span class="">{{ $permissions->total() }}</span>
                    </h5>
                ...
                <div class="flex flex-col flex-shrink-0 space-y-3 md:flex-row md:items-center lg:justify-end md:space-y-0 md:space-x-3">
                    ...
                    <div>
                        <button id="sort-module-dropdownRadioButton" data-dropdown-toggle="sort-module-dropdownRadio" class="w-full md:w-auto flex items-center justify-center py-2 px-4 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:ring-4 focus:ring-gray-200" type="button">
                            Sort By Module
                            <svg class="w-2.5 h-2.5 ms-2.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 4 4 4-4"/>
                            </svg>
                        ...
                        <!-- Dropdown menu -->
                        <div id="sort-module-dropdownRadio" class="z-10 hidden w-48 bg-white divide-y divide-gray-100 rounded-lg shadow-sm " data-popper-reference-hidden="" data-popper-escaped="" data-popper-placement="top" style="position: absolute; inset: auto auto 0px 0px; margin: 0px; transform: translate3d(522.5px, 3847.5px, 0px);">
                            <ul class="p-3 space-y-1 text-sm text-gray-700 " aria-labelledby="sort-module-dropdownRadioButton">
                                @foreach ($modules as $module)
                                    <li>
                                        <div class="flex items-center p-2 rounded-sm hover:bg-gray-100 ">
                                            <input 
                                            wire:model.live="filter_by_module"
                                            id="sort-{{ Str::slug($module) }}" 
                                            type="radio" 
                                            value="{{ $module }}" 
                                            name="filter_by_module" 
                                            class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500   focus:ring-2 ">
                                            <label for="sort-{{ Str::slug($module) }}" class="w-full ms-2 text-sm font-medium text-gray-900 rounded-sm ">{{ $module }}</label>
                                        </div>
                                    </li>
                                @endforeach
                            </ul>
                            ...
                            @foreach ($permissions as $permission) 
                                <tr class="border-b  hover:bg-gray-100 ">
                                      <td class="w-4 px-4 py-3">
                                        <div class="flex items-center">
                                            <input id="record-{{ $permission->id }}" 
                                            wire:model="selected_records"
                                            wire:change="updateSelectedCount"
                                            type="checkbox" 
                                            value="{{ $permission->id }}"
                                            class="w-4 h-4 bg-gray-100 border-gray-300 rounded text-primary-600 focus:ring-primary-500 ">
                                            <label for="record-{{ $permission->id }}" class="sr-only">checkbox</label>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap ">
                                        {{  $permission->updated_at->diffForHumans() ?? null }}
                                    </td>

                                    {{-- <td class="px-4 py-3 flex items-center justify-end">
                                        
                                        <a href="{{ route('permission.edit',['permission' => $permission->id]) }}" class="me-1.5 px-3 py-2 text-xs font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300  ">
                                        ...
                                        wire:confirm.prompt="Are you sure?\n\nType DELETE to confirm|DELETE"
                                        id="record-{{ $permission->id }}-deleteButton"  
