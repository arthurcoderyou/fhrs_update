        ...
        <div class="relative overflow-visible bg-white shadow-md  sm:rounded-lg">
            <div class="flex flex-col px-4 py-3 space-y-3 lg:flex-row lg:items-center lg:justify-between lg:space-y-0 lg:space-x-4">
                <div class="flex items-center flex-1 space-x-4">
                    <h5>
                        <span class="text-gray-500">All Hospice Schedules:</span>
                        <span class="">{{ $hospice_schedules->total() }}</span>
                    </h5>
                ...
                <div class="flex flex-col flex-shrink-0 space-y-3 md:flex-row md:items-center lg:justify-end md:space-y-0 md:space-x-3">
                    ...
                    <a href="{{ route('hospice_schedule.create') }}" 
                    wire:navigate
                ...
                <table class="w-full text-sm text-left text-gray-500  ">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50  ">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3">Date (MM DD YYYY) to Date (MM DD YYYY)</th>   
                    ...
                    <tbody class="overflow-visible">
                        @if(!empty($hospice_schedules) && count($hospice_schedules) > 0)
                            @foreach ($hospice_schedules as $hospice_schedule) 
                                <tr class="border-b  hover:bg-gray-100 ">

                                    <td class="w-4 px-4 py-3">
                                        <div class="flex items-center">
                                            <input id="record-{{ $hospice_schedule->id }}" 
                                            wire:model="selected_records"
                                            wire:change="updateSelectedCount"
                                            type="checkbox" 
                                            value="{{ $hospice_schedule->id }}"
                                            class="w-4 h-4 bg-gray-100 border-gray-300 rounded text-primary-600 focus:ring-primary-500 ">
                                            <label for="record-{{ $hospice_schedule->id }}" class="sr-only">checkbox</label>
                                        </div>
                                    </td>


                                    {{-- <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap max-w-[20rem] text-wrap overflow-x-auto">
                                        {{ $hospice_schedule->name  ?? null }}
                                    </td> --}}
                                            ...
                                            <el-dropdown class="inline-block p-0">
                                                ...
                                                <el-menu anchor="bottom end" popover class="m-0 w-56 origin-top-right rounded-md bg-white p-0 shadow-lg outline outline-1 outline-black/5 transition [--anchor-gap:theme(spacing.2)] [transition-behavior:allow-discrete] data-[closed]:scale-95 data-[closed]:transform data-[closed]:opacity-0 data-[enter]:duration-100 data-[leave]:duration-75 data-[enter]:ease-out data-[leave]:ease-in">
                                                    <div class="py-1">
                                                        ...
                                                        @if(Auth::user()->hasRole('Global Administrator') || Auth::user()->can('hospice schedule edit') )
                                                        <a href="{{ route('hospice_schedule.edit',['hospice_schedule' => $hospice_schedule->id]) }}" 
                                                        wire:navigate 
                                                        class="block px-4 py-2 text-sm text-gray-700 focus:bg-gray-100 focus:text-gray-900 focus:outline-none">
                                                            <div class="flex justify-between items-center">
                                                                <div>
                                                        ...
                                                        @if(Auth::user()->hasRole('Global Administrator') || Auth::user()->can('hospice schedule delete') )
                                                        <!-- Force Delete-->
                                                        <button
                                                            wire:click="confirmDelete({{ $hospice_schedule->id }})"
                                                            type="button"
                                    ...
                                    <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap ">
                                        {{  $hospice_schedule->updated_at->diffForHumans() ?? null }}
                                    </td>
                                    ...
                                    <td class="px-4 py-3 flex items-center justify-end">
                                        ...
                                        @else
                                            <!-- create-funeral-schedule -->
                                                <a href="{{ route('funeral_schedule.create',['hospice_schedule_id' => $hospice_schedule->id]) }}" data-tooltip-target="tooltip-btn-create-funeral-schedule-{{ $hospice_schedule->id }}" 
                                        ...
                                        @endif
                                         ...
                                         <!-- show-hospice-schedule -->
                                            <a href="{{ route('hospice_schedule.show',['hospice_schedule' => $hospice_schedule->id]) }}" 
                                                data-tooltip-target="tooltip-btn-show-hospice-schedule-{{ $hospice_schedule->id }}"
                                            ...
                                            <div id="tooltip-btn-show-hospice-schedule-{{ $hospice_schedule->id }}" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-xs opacity-0 tooltip dark:bg-gray-700">
                                                Show Hospice Schedule
                                                <div class="tooltip-arrow" data-popper-arrow></div>
                                            </div>
                                        <!-- ./ show-hospice-schedule -->
                                        ...
                                        <!-- edit-hospice-schedule -->
                                            <a href="{{ route('hospice_schedule.edit',['hospice_schedule' => $hospice_schedule->id]) }}" 
                                                data-tooltip-target="tooltip-btn-edit-hospice-schedule-{{ $hospice_schedule->id }}"
                                            ...
                                            <div id="tooltip-btn-edit-hospice-schedule-{{ $hospice_schedule->id }}" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-xs opacity-0 tooltip dark:bg-gray-700">
                                                Update Hospice Schedule
                                                <div class="tooltip-arrow" data-popper-arrow></div>
                                            </div>
                                        <!-- ./ edit-hospice-schedule -->


                                        <!-- delete-hospice-schedule -->
                                        <button type="button"  
                                        data-tooltip-target="tooltip-btn-delete-hospice-schedule-{{ $hospice_schedule->id }}" 
                                        wire:click="delete({{ $hospice_schedule->id }})" 
                                        wire:confirm.prompt="Are you sure?\n\nType DELETE to confirm|DELETE"
                                        id="record-{{ $hospice_schedule->id }}-deleteButton"  
                                        ...
                                        <div id="tooltip-btn-delete-hospice-schedule-{{ $hospice_schedule->id }}" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-xs opacity-0 tooltip dark:bg-gray-700">
                                            Delete Hospice Schedule
                                            <div class="tooltip-arrow" data-popper-arrow></div>
                                        </div>
                                        <!-- ./ delete-hospice-schedule -->
            ...
            <nav class="flex flex-col items-start justify-between p-4 space-y-3 md:flex-row md:items-center md:space-y-0" aria-label="Table navigation">
                ...
                <div class="inline-flex items-center gap-x-2">
                    ...
                    <p class="text-sm text-gray-600 ">
                        {{ count($hospice_schedules) > 0 ? 'of '.$hospice_schedules->total()  : '' }}
                    </p>
                </div>
            </nav>
        </div>
