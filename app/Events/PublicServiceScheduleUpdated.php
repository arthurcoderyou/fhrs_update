...
class PublicServiceScheduleUpdated implements ShouldBroadcastNow
{
    ...
    public function __construct(ServiceSchedule $service_schedule)
    {
        ...
        if ($this->service_schedule) {
            $this->message = "{$userName} has updated the service schedule for '{$this->service_schedule->name_of_person}'";
        } else {
            $this->message = "{$userName} has updated multiple service schedules";
        }
